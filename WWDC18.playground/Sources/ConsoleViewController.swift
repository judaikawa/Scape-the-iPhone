...
//    , "How did I ended up in\nhere?!?"
    ...
    public override func loadView() {
        ...
        do{
            ...
            bButtonPlayer.volume = 0.5
            bButtonPlayer.prepareToPlay()
        }
        catch{
        }

        // TESTE - End of game by passing through all the apps
...
//        passedInMessagesApp = true
    ...
    @objc func aButtonPressed() {
        ...
        if startGoToMenu {
            ...
            textOptionLabel.frame = CGRect(x: 30, y: 40, width: 170, height: 170)
        ...
        if let selectedApp = checkSelectedApp() {
            ...
            if passedInAllApps && self.inInAppVC is CameraViewController {
                ...
                UIView.animate(withDuration: 2.0, animations: {
                    ...
                    if finished {
                        // self.view.addSubview(CalculatorViewController().view)
...
//                        self.aButton.isEnabled = true
    ...
    func checkSelectedApp() -> UIViewController? {
        ...
        if character.frame.origin.x >= (notesApp.frame.minX-10) &&
                  character.frame.origin.x <= (notesApp.frame.maxX-10) &&
                  character.frame.origin.y >= (notesApp.frame.minY-character.frame.size.height) &&
                  character.frame.origin.y <= (notesApp.frame.maxY-character.frame.size.height) {
            return NotesViewController()
            
        } else if character.frame.origin.x >= (calculatorApp.frame.minX-10) &&
                  ...
                  character.frame.origin.y <= (calculatorApp.frame.maxY-character.frame.size.height) {
            return CalculatorViewController()
            
        } else if character.frame.origin.x >= (messagesApp.frame.minX-10) &&
            ...
            character.frame.origin.y <= (messagesApp.frame.maxY-character.frame.size.height) {
            return MessagesViewController()
            
        } else if character.frame.origin.x >= (cameraApp.frame.minX-10) &&
            ...
            character.frame.origin.y <= (cameraApp.frame.maxY-character.frame.size.height) {
            return CameraViewController()
            
        } else if character.frame.origin.x >= (photosApp.frame.minX-10) &&
            ...
            character.frame.origin.y <= (photosApp.frame.maxY-character.frame.size.height) {
            return PhotosViewController()
            
        } else if character.frame.origin.x >= (clockApp.frame.minX-10) &&
            character.frame.origin.x <= (clockApp.frame.maxX-10) &&
            character.frame.origin.y >= (clockApp.frame.minY-character.frame.size.height) &&
            character.frame.origin.y <= (clockApp.frame.maxY-character.frame.size.height) {
            return ClockViewController()
            
        } else if character.frame.origin.x >= (mailApp.frame.minX-10) &&
            character.frame.origin.x <= (mailApp.frame.maxX-10) &&
            character.frame.origin.y >= (mailApp.frame.minY-character.frame.size.height) &&
            character.frame.origin.y <= (mailApp.frame.maxY-character.frame.size.height) {
            return MailViewController()
            
        } else if character.frame.origin.x >= (calendarApp.frame.minX-10) &&
            ...
            character.frame.origin.y <= (calendarApp.frame.maxY-character.frame.size.height) {
